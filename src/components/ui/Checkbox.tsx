...
export default function Checkbox(props: CheckboxProps) {
  ...
  delete (inputProps as { dangerouslySetInnerHTML?: unknown }).dangerouslySetInnerHTML
  ...
  useEffect(() => {
    if (inputRef.current) {
      inputRef.current.indeterminate = Boolean(indeterminate) && !currentChecked
    }
    ...
    >
      ...
      {(label || description || error) && (
        <span className="flex flex-col gap-0.5">
          ...
          {error && (
            <span id={errorId} className="text-xs text-[var(--color-brand-default)]">
              {error}
            </span>
          )}
        </span>
